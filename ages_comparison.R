...
#     age.groups.ids.tissues[[tissue]]$fetus.samples.ids = fetus.samples.ids
#     age.groups.ids.tissues[[tissue]]$adult.samples.ids = adult.samples.ids
#   }
#   age.groups.ids.tissues
# }
# 
...
#   
#   for (tissue in unique.tissues){
#     fetus.ids = age.groups.ids.tissues[[tissue]]$fetus.samples.ids
#     adult.ids = age.groups.ids.tissues[[tissue]]$adult.samples.ids
#     
...
#     jxns.sajr.ids.passed = unique(c(jxns.sajr.ids.passed, rownames(sajr.age.pairs$ir)))
#   }
...
# 
#   sajr.filtered = sajr[sajr.jxns.ids.passed, c(fetus.ids,adult.ids)]
#   rse.filtered = rse.ERP109002.jxn.cytosk.genes[rse.jxns.ids.passed, c(adult.ids,fetus.ids)]
#   
#   mod = c(rep("fetus", length(fetus.ids)),rep("adult", length(adult.ids)))
#   mod = list(age=factor(mod)) # ~ model data
#   
#   list(sajr.filtered=sajr.filtered, mod=mod, rse.filtered=rse.filtered)
# }
# 
# calculateMetrics = function(sajr.filtered, fetus.samples.ids, adult.samples.ids){
#   dPSI = apply(sajr.filtered$ir, 1, function(x) mean(x[fetus.samples.ids],na.rm=T)-mean(x[adult.samples.ids],na.rm=T))
...
# runSAJR = function(){
#   sajr.output = list()
#   for (tissue in unique.tissues){  # for every tissue
...
#                                age.groups.ids.tissues[[tissue]]$fetus.samples.ids, 
#                                age.groups.ids.tissues[[tissue]]$adult.samples.ids)
...
#     anlz.out = makeDJexpress(rse.filtered, reference.condition='fetus', 
...
#     dje.output[[tissue]] = anlz.out
#     print(c(tissue,dim(anlz.out)))
...
#     prepared.data = prepareData(tissue)
#     rse.filtered = prepared.data$rse.filtered
#     makeDiegoInputFiles(rse.filtered,tissue)
#     
...
#   for (tissue in unique.tissues) {
#     # Construct file path
#     file_path = paste0("/home/an/DIEGO_output_files/DIEGO_output", tissue)  # Replace with actual path
#     
...
#     sajr.dje.common.jxns = merge(sajr.output[[tissue]], dje.output[[tissue]],
#                             by = "unifiedJxnID", all = TRUE) # inner join, duplicated rows will remain
#     
#     all.common.jxns = merge(sajr.dje.common.jxns, diego.output[[tissue]],
#                                    by = "unifiedJxnID", all = TRUE) # inner join, duplicated rows will remain
#     
#     # -- finding significant
#     # significant events
...
# 
#     # saving to the list
#     output[[tissue]]$all.common.jxns = all.common.jxns
#     output[[tissue]]$sajr.dje.significant.events = sajr.dje.significant.events
#     output[[tissue]]$sajr.diego.significant.events = sajr.diego.significant.events
#     output[[tissue]]$dje.diego.significant.events = dje.diego.significant.events
#     
#     output[[tissue]]$sajr.only.significant.events = sajr.only.significant.events
#     output[[tissue]]$dje.only.significant.events = dje.only.significant.events
#     output[[tissue]]$diego.only.significant.events = diego.only.significant.events
#     output[[tissue]]$sign.all.tools = sign.all.tools
# 
#   }
#   output
# }
...
output = readRDS('output.rds', refhook = NULL)
